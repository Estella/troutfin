...
int is_chan_ophalf(aClient *cptr, aChannel *chptr)
{
    chanMember   *cm;
        ...
        if (ismine)
        {
            ...
            if (is_quieted(cptr, chptr, NULL))
                return (MODE_BAN); /*
                                ...
                                */
            if (call_hooks(CHOOK_CHECKCANSPEAK, cptr, chptr, msg))
                return (MODE_BAN);
            if (call_hooks(CHOOK_CHECKCANJOIN, cptr, chptr))
                return (MODE_BAN);
        }
    }
    else
...
{
    chanMember       *l, *anop = NULL, *ahalf, *skip = NULL;
    int         n = 0;
    char       *t;
    ...
    *mbuf++='+'; /* add the plus, even if they don't */
    ...
    while(*modes) 
    {
        ...
        case 'I':
            ...
            if (parv[args][0] == '$') extban = 1;

            if (extban) {
                if (parv[args][2] != ';') {
                    args++; // Incorrect delimiter
                    break;
                }
            }
...
int m_kick(aClient *cptr, aClient *sptr, int parc, char *parv[])
{
    ...
    *nickbuf = *buf = '\0';
    ...
    while (name)
    {
        ...
        if (!IsServer(sptr) && !is_chan_ophalf(sptr, chptr) && !IsULine(sptr))
        {
            // Good fucking grief!@# Now that we have halfops (this is ircd-rainbow-trout)
            // we have to make sure we're not allowing a half to kick a full. Scheisse.
            // Actually, let's check this further down.
